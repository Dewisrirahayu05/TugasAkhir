...
if menu == "Visualisasi & Hasil Klaster Wilayah":
    ...
    with tab_tabel:
        ...
        # DOWNLOAD DATA
        # ----------------------------------------------------------------------
        # Mengubah dataframe menjadi format CSV agar dapat diunduh.
...
# ==============================================================================
    ...
    if st.button("Jalankan Proses Analisis Sistem", use_container_width=True):
        if (ipm is None) or (p0 is None) or (p1 is None) or (p2 is None) or (gk is None) or (gr is None):
                st.warning("**Mohon lengkapi semua indikator wajib diisi terlebih dahulu sebelum menjalankan simulasi prediksi!**")
        elif not (0.0 <= ipm <= 100.0) or not (0.0 <= p0 <= 100.0) or not (0.0 <= gr <= 1.0):
                ...
                try:
                    ...
                    st.error(
                        f"Sistem gagal melakukan pemrosesan prediksi: {str(error_sistem)}"
                    )
